...
returnRouter.get("/profileMaterialFirst", async (req, res, next) => {
  try {
    ...
    misQueryMod(
      `SELECT m1.Rv_date,m.Mtrl_Rv_id, m1.Cust_Code, m1.RV_No, m1.CustDocuNo as Cust_Docu_No, m.Mtrl_Code, 
      m.DynamicPara1, m.DynamicPara2, m.DynamicPara3, m.Scrap, 
    ...
    group by m1.Rv_date,m.Mtrl_Rv_id, m1.Cust_Code, m1.RV_No, m1.CustDocuNo, m.Mtrl_Code, m.DynamicPara1, 
    m.DynamicPara2, m.DynamicPara3, m.Scrap`,
      (err, data) => {
...
returnRouter.get("/profileMaterialSecond", async (req, res, next) => {
  try {
    ...
    misQueryMod(
      `
      SELECT m1.RVId,m.Mtrl_Rv_id, m1.Cust_Code, m1.RV_No, m1.CustDocuNo as Cust_Docu_No, m.Mtrl_Code, m.DynamicPara1, m.DynamicPara2, m.DynamicPara3, 
		m.Scrap, m.Weight,
      m.ScrapWeight,m.MtrlStockID, Issue
      FROM magodmis.mtrlstocklist m,
        magodmis.material_receipt_register m1
      WHERE  (m.Scrap or  not m.Locked) AND m.IV_No is null  AND m.Cust_Code=${Cust_Code}
      AND m1.Rv_No=m.rv_No AND m1.RVStatus='Received'`,
      (err, data) => {
        if (err) logger.error(err);
        res.send(data);
      }
...
returnRouter.get("/partSecond", async (req, res, next) => {
  try {
    let Cust_Code = req.query.Cust_Code;
    misQueryMod(
      `SELECT m1.* FROM magodmis.material_receipt_register m,magodmis.mtrl_part_receipt_details m1
